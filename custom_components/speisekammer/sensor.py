...
async def async_setup_entry(hass, entry, async_add_entities):
    """Setup Sensoren über ConfigEntry."""
...
class ExpiringItemsSensor(Entity):
    ...
    async def async_update(self):
        ...
        for item in items:
            exp_date_str = item.get("bestBeforeDate") or item.get("expiryDate")
            if not exp_date_str:
                continue
            try:
        ...
        self._attributes = {
            "items": [{"name": i.get("name"), "expires": i.get("bestBeforeDate")} for i in expiring]
        }
